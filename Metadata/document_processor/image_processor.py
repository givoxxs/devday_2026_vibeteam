...
CAPTION_PROMPT = """You are analyzing a document page from a Japanese solar power plant (VPP) construction project.

Respond ONLY with this JSON (no extra text):
{
  "caption": "One sentence in English describing what this document is",
  "description": "3-4 sentences describing the document content: what it covers, key entities mentioned, purpose, and any notable values or dates",
  "document_type": "e.g. Test Report / Equipment Spec / Warranty / Grid Application / Construction Photo",
  "key_info": ["up to 5 items: dates, equipment names, measured values, project name, serial numbers"],
  "folder_number": 18,
...
Folder numbers (choose EXACTLY one):
1=Cover/Spine, 2=TOC, 3=Handover List, 4=Completion Report, 5=Schedule,
6=As-built Drawings, 7=Test Reports/Inspection Sheets, 8=Self-Inspection,
10=Equipment Config/List, 11=PCS/Power Conditioners, 12=Solar Modules,
13=Monitoring/Communication, 14=Equipment Specs, 15=User Manuals,
16=Admin Docs, 17=Utility/Grid Docs, 18=Warranties, 19=Construction Photos,
20=Structural Calc, 22=Other/Manifest

NOTE: folder 9 and 21 do NOT exist. Warranties → 18, not 7 or 21."""
            ...
            if raw_response:
                ...
                if json_match:
                    try:
                        ...
                        doc.image_summary = parsed.get("description") or parsed.get("document_type")
                        ...
                        elif doc.image_caption:
                            parts.append(doc.image_caption)
                        if key_info:
                            parts.extend(str(k) for k in key_info)
                        doc.full_text = "\n".join(parts)
